<script lang="ts">
	...
	export let type: InputType = "text",
		...
		checked = false,
		dataVault = "",
		initial: string | number | undefined = undefined,
		onChange: ((e: Event) => void) | undefined = undefined,
		onInput: ((e: Event) => void) | undefined = undefined,
		onFocus: ((e: Event) => void) | undefined = undefined,
	...
	let hljs;
	let codevalue: undefined | Writable<string>;
	...
	async function loadHighlightJs(): Promise<void> {
		...
		hljs.registerLanguage("sql", syntax.default);
	}
			...
			<ConditionalWrapper wrap={code} class="textareahighlightcodediv">
				...
				{#if code}
					<pre><code
							...
							on:input={(e) => ($codevalue = e.target.value)}
							>{@html $codevalue}</code
						></pre>
				{/if}
			</ConditionalWrapper>
