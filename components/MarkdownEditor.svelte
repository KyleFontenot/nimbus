...
import Image from "$components/markdownComponents/Image.svelte";
...
export const carta = new Carta({
	sanitizer: (input) =>
		DOMPurify.sanitize(input, { ADD_ATTR: WHITELISTED_PROPS }),
	extensions: [
		customRenderer([
			{
				querySelector: "img",
...
<script lang="ts">
  ...
  export let 
    ...
    carta: Carta | undefined = undefined;
...
<style lang="scss">
  ...
  :global(div.carta-renderer), :global(div.carta-viewer) {
    // & :global(h1) {
    // }
    // & :global(p) {
    // }
